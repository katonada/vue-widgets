      ...
      <div class="ecom__container">
        ...
        <div class="ecom__data">
          ...
          <AddToCart
            ...
            :type="'bundle'"
            :activeBundleTotalPrice="this.activeBundleTotalPrice"
            class="ecom__atc"
          />
    ...
    calculateSavings() {
      if (this.product.variations[this.activeVariation].properties.length) {
        for (let i = 0; i < this.product.variations.length; i++) {
          let maskQuantity = this.product.variations[i].properties[0].quantity
          let maskBundlePrice = this.product.variations[i].price.original.number
          let maskOriginalPrice = this.product.variations[i].properties[0].variations[0].price.original.number

          let bundleSavings = (maskBundlePrice - (maskOriginalPrice) * maskQuantity).toFixed(2)

          // Savings is negative value so we subtract it from bundle price to get total value.
          let totalBundlePrice = maskBundlePrice - bundleSavings
